...
export default function AdminPublicApplicationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const router = useRouter();
  const [app, setApp] = useState<PublicAppData | null>(null);
  ...
  useEffect(() => {
    async function load() {
      ...
      setApp(data as unknown as PublicAppData);
  ...
  async function handleReject() {
    ...
    setRejecting(true);
    setFeedback("");
    const result = await rejectPublicApplication(appId, rejectComment);
    setRejecting(false);
    if (result.error) {
      setFeedback("Error: " + result.error);
    } else {
  ...
  async function saveInternalComment() {
    const supabase = createClient();
    await supabase.from("public_applications").update({ admin_comment: internalComment }).eq("id", appId);
  ...
  return (
    <div className="p-6 md:p-10 max-w-[800px]">
      ...
      {/* Source badge */}
      <div className="mb-4">
        <span className="px-2.5 py-1 rounded-full text-[10px] uppercase tracking-[0.06em] text-blue-300 bg-blue-400/10">
          Postulación pública (wizard)
        </span>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="text-[13px] text-brand-smoke">
          {app.host_name || "Sin marca"} · {app.location} · {date}
        </div>
      </div>
      ...
      <div className="mb-6 p-5 rounded-xl border border-blue-400/20 bg-blue-400/[0.03]">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-[13px]">
          <div>
            ...
            <p className="text-brand-white">{app.email}</p>
          </div>
          <div>
            <div className={labelClass}>Teléfono (WhatsApp)</div>
            <p className="text-brand-white">{app.phone}</p>
          </div>
          <div>
            <div className={labelClass}>Host / Marca</div>
      ...
      <div className="space-y-6 mb-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div>
            <div className={labelClass}>Contacto comercial</div>
          ...
          <div>
            <div className={labelClass}>Exclusividad</div>
            <p className="text-[13px] text-brand-white">{app.exclusivity_confirmed ? "Confirmada" : "No confirmada"}</p>
          </div>
        </div>
